...
{		
	...
	private String buttonText = new TranslationTextComponent("gui.button.pandoras_creatures.curseforge").getString();
	...
	public void renderButton(MatrixStack matrix, int mouseX, int mouseY, float partial)
	{
		if(visible && Config.CLIENT.shouldButtonsInCreativeTabBeEnabled.get() == true && Minecraft.getInstance().player.getActivePotionEffects().isEmpty())
		{
			...
			if(isHovered)
			{
			    Minecraft.getInstance().currentScreen.renderToolTip(matrix, this.list, x - (15 + fontRenderer.getStringWidth(this.buttonText)), y + 19, this.fontRenderer);
			}
		}
		else
		{
