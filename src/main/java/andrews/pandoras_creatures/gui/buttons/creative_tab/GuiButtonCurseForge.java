...
{		
	...
	public void renderButton(MatrixStack matrix, int mouseX, int mouseY, float partial)
	{
		if(this.creativeTab.getSelectedTabIndex() == PandorasCreatures.PANDORAS_CREATURES_GROUP.getIndex() && PCConfigs.PCClientConfig.shouldButtonsInCreativeTabBeEnabled.get() == true && Minecraft.getInstance().player.getActivePotionEffects().isEmpty())
		{
			...
			GuiUtils.drawTexturedModalRect(matrix, x, y, u, v, width, height, 0);
			...
			if(isHovered)
			{
				Minecraft.getInstance().currentScreen.renderToolTip(matrix, Arrays.asList(this.buttonText.func_241878_f()), x - (15 + fontRenderer.getStringWidth(this.buttonText.getString())), y + 19, this.fontRenderer);
			}
		}
		else
		{
