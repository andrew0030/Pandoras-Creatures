...
public class EndTrollBulletDamageEntity extends Entity
{
	...
	public EndTrollBulletDamageEntity(World worldIn, LivingEntity ownerIn, Entity targetIn, Direction.Axis directionAxis)
	{
		this(PCEntities.END_TROLL_BULLET_DAMAGE.get(), worldIn);
      	this.owner = ownerIn;
	...
	public void tick()
	{
		...
		this.setPosition(this.getPosX() + vec3d1.x, this.getPosY() + vec3d1.y, this.getPosZ() + vec3d1.z);
     	...
     	if(this.world.isRemote)
     	{
     		this.world.addParticle(new RedstoneParticleData(141, 0, 0, 1.0F), this.getPosX() - vec3d1.x, this.getPosY() - vec3d1.y + 0.15D, this.getPosZ() - vec3d1.z, 0.0D, 0.0D, 0.0D);
     	}
	...
	protected void bulletHit(RayTraceResult result)
	{
		if(result.getType() == RayTraceResult.Type.ENTITY)
		{
			...
			if(flag)
			{
				...
				{
					((LivingEntity)entity).attackEntityFrom(DamageSource.causeIndirectDamage(this, this.owner).setProjectile(), 8.0F);
				}
			}
		}
		else
