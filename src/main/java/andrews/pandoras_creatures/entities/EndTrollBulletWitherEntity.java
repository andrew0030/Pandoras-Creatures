...
public class EndTrollBulletWitherEntity extends Entity
{
	...
	public EndTrollBulletWitherEntity(World worldIn, LivingEntity ownerIn, Entity targetIn, Direction.Axis directionAxis)
	{
		this(PCEntities.END_TROLL_BULLET_WITHER.get(), worldIn);
      	this.owner = ownerIn;
	...
	public void tick()
	{
		...
		this.setPosition(this.getPosX() + vec3d1.x, this.getPosY() + vec3d1.y, this.getPosZ() + vec3d1.z);
     	...
     	if(this.world.isRemote)
     	{
     		this.world.addParticle(new RedstoneParticleData(0, 0, 0, 1.0F), this.getPosX() - vec3d1.x, this.getPosY() - vec3d1.y + 0.15D, this.getPosZ() - vec3d1.z, 0.0D, 0.0D, 0.0D);
     	}
