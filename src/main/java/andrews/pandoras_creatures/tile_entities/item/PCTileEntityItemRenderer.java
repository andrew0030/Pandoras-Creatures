...
import andrews.pandoras_creatures.registry.PCTags.Items;
...
{	
	...
	public void func_239207_a_(ItemStack itemStack, TransformType type, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay)
	{
		if(itemStack.getItem() == PCBlocks.PANDORIC_SHARD.get().asItem())
		{
			...
			{
				PandoricShardTileEntity shardTileEntity = (PandoricShardTileEntity) this.te.get();//TODO make it so it gets the variant from the stacks NBT
				shardTileEntity.setShardSize(2);
				shardTileEntity.setShardVariant(3);
